...
@section('title', 'Edit Member — Slytherin Restricted Library')
...
<div class="relative z-10 max-w-2xl">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-2 mb-3 text-xs" style="font-family:'Inter',sans-serif;">
            ...
            <span style="color:rgba(192,192,192,0.45);">Edit Anggota</span>
        </div>
        ...
        <div style="display:flex; align-items:center; margin: 6px 0 20px;">
            <svg width="200" height="12" viewBox="0 0 200 12" fill="none">
                <defs>
                    <linearGradient id="dv2" x1="0" y1="0" x2="160" y2="0" gradientUnits="userSpaceOnUse">
                ...
                <line x1="24" y1="6" x2="200" y2="6" stroke="url(#dv2)" stroke-width="0.75"/>
            </svg>
        </div>

        <p style="font-family:'IM Fell English',serif; font-style:italic; font-size:14px; color:rgba(134,239,172,0.65);">
            Perbarui catatan jiwa: <span style="color:#86efac;">{{ $member->nama_member }}</span>
        </p>
    </div>
    ...
    <div class="form-card">
        ...
        <div class="flex items-center gap-4 mb-6" style="position:relative;z-index:2;">
            @if ($member->foto)
            ...
            @else
                <div class="w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0"
                    ...
                    <i class="ti ti-user text-xl" style="color:rgba(212,175,55,0.45);"></i>
                </div>
            @endif
            <div>
                <p style="font-family:'IM Fell English',serif; font-size:16px; color:#f5f0e8;">{{ $member->nama_member }}</p>
                <p style="font-size:12px; color:rgba(134,239,172,0.6); font-family:'Inter',sans-serif;">{{ $member->nomor_member }}</p>
            </div>
            <div class="flex-1 h-px ml-2" style="background: linear-gradient(90deg, rgba(212,175,55,0.2), transparent);"></div>
        </div>

        <form action="{{ route('member.update', $member->id_member) }}" method="POST" class="space-y-5"
            ...
            <div>
                ...
                <div class="input-wrap textarea-wrap">
                    ...
                    <textarea name="alamat" id="alamat" rows="3" class="input-field">{{ old('alamat', $member->alamat) }}</textarea>
                </div>
                @error('alamat')
                    <p class="text-xs mt-1.5 flex items-center gap-1.5" style="color:#fca5a5;"><i class="ti ti-alert-circle text-xs"></i>{{ $message }}</p>
                @enderror
                ...
                <div>
                    ...
                    <div class="input-wrap">
                        ...
                        <input type="date" name="tgl_terkahir_bayar" id="tgl_terkahir_bayar"
                               value="{{ old('tgl_terkahir_bayar', $member->tgl_terkahir_bayar?->format('Y-m-d')) }}"
                               class="input-field">
                    </div>
            ...
            <div>
                <label class="field-label">Ganti Foto <span style="color:rgba(192,192,192,0.35);font-size:9px;">(opsional)</span></label>
                <div class="file-drop" id="fileDropZone">
                    ...
                    <div id="fileDropContent">
                        ...
                        <p style="color:rgba(245,240,232,0.5); font-size:13px;">Klik untuk mengganti foto</p>
                        <p style="color:rgba(192,192,192,0.3); font-size:11px; margin-top:4px;">JPG / PNG · Maks. 2MB · Kosongkan jika tidak ingin mengganti</p>
                    </div>
                </div>
                @error('foto')
                    <p class="text-xs mt-1.5 flex items-center gap-1.5" style="color:#fca5a5;"><i class="ti ti-alert-circle text-xs"></i>{{ $message }}</p>
