...
@section('title', 'Edit Peminjaman — Slytherin Restricted Library')
...
<style>
    ...
    .status-returned { background: rgba(212,175,55,0.12); border: 1px solid rgba(212,175,55,0.3); color: #d4af37; }
    .status-active   { background: rgba(34,197,94,0.12);  border: 1px solid rgba(34,197,94,0.3);  color: #86efac; }
...
<div class="relative z-10 max-w-2xl">
    ...
    <div class="mb-8">
        ...
        <div style="display:flex; align-items:center; margin: 6px 0 20px;">
            <svg width="200" height="12" viewBox="0 0 200 12" fill="none">
                ...
                <line x1="24" y1="6" x2="200" y2="6" stroke="url(#dv6)" stroke-width="0.75"/>
            </svg>
        </div>

        <div class="flex items-center gap-2">
            <p style="font-family:'IM Fell English',serif; font-style:italic; font-size:14px; color:rgba(134,239,172,0.65);">
                Transaksi #{{ $peminjaman->id_peminjaman }}
            </p>
            @if ($peminjaman->tgl_kembali)
                <span class="status-pill status-returned"><i class="ti ti-check text-xs"></i> Dikembalikan</span>
            @else
                <span class="status-pill status-active"><i class="ti ti-clock text-xs"></i> Sedang Dipinjam</span>
            @endif
        </div>
    </div>
    ...
    <div class="form-card">
        ...
        {{-- Current summary box --}}
        <div class="summary-box mb-6" style="position:relative;z-index:2;">
            <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0"
                 style="border:1px solid rgba(212,175,55,0.3); background:rgba(212,175,55,0.06);">
                <i class="ti ti-receipt text-base" style="color:rgba(212,175,55,0.6);"></i>
            </div>
            <div style="flex:1;">
                <p style="font-size:11px; color:rgba(192,192,192,0.45); font-family:'Inter',sans-serif; letter-spacing:0.06em; text-transform:uppercase; margin-bottom:2px;">Peminjam saat ini</p>
                <p style="font-family:'IM Fell English',serif; font-size:15px; color:#f5f0e8;">{{ $peminjaman->member->nama_member ?? '—' }}</p>
                <p style="font-size:11px; color:rgba(134,239,172,0.5); font-family:'Inter',sans-serif; margin-top:1px;">{{ $peminjaman->buku->judul ?? '—' }}</p>
            </div>
        </div>

        <form action="{{ route('peminjaman.update', $peminjaman->id_peminjaman) }}" method="POST" class="space-y-5"
                    ...
                    <select name="id_member" id="id_member" class="input-field">
                        ...
                        @foreach ($members as $member)
                            <option value="{{ $member->id_member }}"
                                @selected(old('id_member', $peminjaman->id_member) == $member->id_member)>
            ...
            <div>
                <label for="id_buku" class="field-label">Grimoire</label>
                <div class="input-wrap">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1.5">
                        ...
                        @if ($peminjaman->tgl_kembali)
                            <span class="status-pill status-returned" style="font-size:9px; padding:2px 7px;">
                                <i class="ti ti-check" style="font-size:9px;"></i> Sudah
                            </span>
                        @else
                            <span class="status-pill status-active" style="font-size:9px; padding:2px 7px;">
                                <i class="ti ti-clock" style="font-size:9px;"></i> Belum
                            </span>
                        @endif
                    </div>
                    <div class="input-wrap">
